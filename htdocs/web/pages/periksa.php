...
// Query untuk mendapatkan data jadwal_periksa berdasarkan daftar_poli untuk dokter yang sedang login
$sql = "
    SELECT dp.id, d.nama as dokter, jp.hari, jp.jam_mulai, jp.jam_selesai 
    FROM daftar_poli dp
...
$queryObat = "SELECT id, nama_obat FROM obat";
...
// Menyimpan data obat ke dalam array
$obatOptions = [];
while ($rowObat = mysqli_fetch_assoc($resultObat)) {
    $obatOptions[] = $rowObat;
}

// Pastikan form telah dikirimkan (misalnya, dengan menggunakan isset($_POST))
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Ambil data dari formulir
    $id_pasien = $_POST['id']; // Contoh, sesuaikan dengan input yang sesuai
    $tanggal_periksa = $_POST['tanggal_periksa'];
    $catatan = $_POST['catatan'];
    $obat = $_POST['obat']; // Ini akan berisi array karena menggunakan multiple select

    // Lakukan validasi data jika diperlukan

    // Simpan data ke dalam tabel periksa atau melakukan tindakan lainnya
    // Contoh penyimpanan data menggunakan PDO (asumsikan Anda telah mengatur koneksi sebelumnya)
    $sql = "INSERT INTO periksa (id_daftar_poli, tgl_periksa, catatan, biaya_periksa) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isss", $id_daftar_poli, $tanggal_periksa, $catatan, $biaya_periksa);    
    ...
    if ($stmt->execute()) {
        ...
        // Simpan data obat ke dalam tabel periksa_obat (asumsikan ini tabel relasinya)
        foreach ($obat as $id_obat) {
            $sql_obat = "INSERT INTO detail_periksa (id_periksa, id_obat) VALUES (?, ?)";
            $stmt_obat = $conn->prepare($sql_obat);
            $stmt_obat->bind_param("ii", $periksa_id, $id_obat);
            $stmt_obat->execute();
        }
  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="../pages/periksa.php">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                                ...
                                <tbody>
                                    ...
                                    if ($result->num_rows > 0) {
                                        while($row = $result->fetch_assoc()) {
                                            ...
                                            echo "<button class='btn btn-link text-dark px-3 mb-0' data-bs-toggle='modal' data-bs-target='#periksaModal' data-id='" . $row['id'] . "'><i class='fas fa-stethoscope text-dark me-2' aria-hidden='true'></i>Periksa</button>";
                                            echo "</td>";
                                            echo "</tr>";
                                        }
                                    } else {
...
<!-- Modal Form untuk Periksa -->
<div class="modal fade" id="periksaModal" tabindex="-1" aria-labelledby="periksaModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="periksaModalLabel">Hasil Pemeriksaan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="periksaForm" action="periksa.php" method="POST">
    <div class="modal-body">
        <input type="hidden" id="periksaId" name="id">
        <div class="mb-3">
            <label for="dokterName" class="form-label">Dokter</label>
            <p id="dokterName" class="form-control-plaintext"><?php echo $_SESSION['full_name']; ?></p>
        </div>
        <div class="mb-3">
            <label for="tanggalPeriksa" class="form-label">Tanggal Periksa</label>
            <input type="date" class="form-control" id="tanggalPeriksa" name="tanggal_periksa" required>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-control select2" id="obat" name="obat[]" multiple required>
                <?php foreach ($obatOptions as $obat) { ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var periksaModal = document.getElementById('periksaModal');
        periksaModal.addEventListener('show.bs.modal', function (event) {
            var button = event.relatedTarget;
            var id = button.getAttribute('data-id');

            var modalId = periksaModal.querySelector('#periksaId');
            modalId.value = id;
        });
    });
</script>
...
<script>
    $(document).ready(function() {
        ...
        // Refresh select2 saat modal ditampilkan
