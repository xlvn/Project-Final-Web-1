...
// Proses formulir jika ada permintaan POST
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Query untuk memperbarui jadwal
    $sql_update = "UPDATE jadwal_periksa SET hari=?, jam_mulai=?, jam_selesai=? WHERE id=?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("sssi", $hari, $jam_mulai, $jam_selesai, $id);
  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link  " href="../pages/periksa.php">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                                ...
                                <tbody>
                                    ...
                                    if ($result->num_rows > 0) {
                                        while($row = $result->fetch_assoc()) {
                                            ...
                                            echo "<button class='btn btn-link text-dark px-3 mb-0' data-bs-toggle='modal' data-bs-target='#editModal' data-id='" . $row['id'] . "' data-dokter='" . $row['dokter'] . "' data-hari='" . $row['hari'] . "' data-jam-mulai='" . $row['jam_mulai'] . "' data-jam-selesai='" . $row['jam_selesai'] . "'><i class='fas fa-pencil-alt text-dark me-2' aria-hidden='true'></i>Edit</button>";
                                            echo "</td>";
                                            echo "</tr>";
                                        }
                                    } else {
...
<!-- Modal Form untuk Mengedit Jadwal -->
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="editForm" action="jadwal.php" method="POST">
                <div class="modal-body">
                    <input type="hidden" id="editId" name="id">
                    <div class="mb-3">
                        <label for="editDokter" class="form-label">Dokter</label>
                        <input type="text" class="form-control" id="editDokter" name="dokter" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="editJamMulai" name="jam_mulai" required>
                    </div>
                    <div class="mb-3">
...
<!-- Tambahkan Tombol Edit di Setiap Baris Jadwal -->
<?php
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        echo "<tr>";
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    editModal.addEventListener('show.bs.modal', function (event) {
        ...
        var jamMulai = button.getAttribute('data-jam-mulai');
        var jamSelesai = button.getAttribute('data-jam-selesai');

        var modalId = editModal.querySelector('#editId');
        var modalDokter = editModal.querySelector('#editDokter');
        var modalHari = editModal.querySelector('#editHari');
        var modalJamMulai = editModal.querySelector('#editJamMulai');
        var modalJamSelesai = editModal.querySelector('#editJamSelesai');

        modalId.value = id;
        modalDokter.value = dokter;
        modalHari.value = hari;
        modalJamMulai.value = jamMulai;
        modalJamSelesai.value = jamSelesai;
    });
});
</script>
